...
void CMemorandumSystemDlg::OnBnClickedButton4()
{
	...
	{    
		if (IDOK == dlg.DoModal())
		{
			CInfo Info(atoi(dlg.m_id), dlg.m_lastname.GetBuffer(),
	...
	if (IsOpen)
	{
		...
		else
		{
			...
			str.Format(TEXT("查找成功！\n\n编号：%d\n姓氏：%s\n名字：%s\n日期：%s\n内容：%s"),
				FindInfo.m_id, FindInfo.m_lastname.c_str(), FindInfo.m_firstname.c_str(),
				FindInfo.m_date.c_str(), FindInfo.m_content.c_str());
